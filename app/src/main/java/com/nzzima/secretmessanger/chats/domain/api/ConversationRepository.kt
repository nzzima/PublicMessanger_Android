...
interface ConversationRepository {
    ...
    /**
     * Стирает шапку диалога [convoId] — **последним шагом** удаления, после подколлекций.
     *
     * Порядок обязателен: правила на `messages`, `audio` и `images` читают состав участников
     * из шапки через `get()`, а `get()` по несуществующему документу роняет проверку целиком.
     * Стёртая первой шапка оставила бы вложения в базе навсегда — недоступными ни для чтения,
     * ни для удаления **никому**, включая владельца проекта.
     */
    suspend fun erase(convoId: String): Result<Unit>
}
