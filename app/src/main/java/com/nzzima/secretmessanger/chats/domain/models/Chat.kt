...
data class Chat(
    ...
    /**
     * Вправе ли мы стереть переписку у всех.
     *
     * Диалог на двоих стирает любой из двоих: хозяина у него нет, переписка общая. Группу —
     * только создатель, тот же единственный, кто правит её состав.
     *
     * Считается по **числу участников**, а не по [isGroup]: группа, ужавшаяся до двоих,
     * становится диалогом на двоих и стирается любым из оставшихся. Группа, из которой ушли
     * все, кроме создателя, остаётся его — из одного участника пары не выходит.
     *
     * Ровно та же формула стоит правилом `erasable()` в базе. Здесь она про вид строки, там
     * — настоящая; разъехаться им нельзя.
     */
    val canErase: Boolean get() = members.size == 2 || owner == selfId
