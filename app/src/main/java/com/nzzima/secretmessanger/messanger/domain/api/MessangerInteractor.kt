...
interface MessangerInteractor {
    ...
    /**
     * Выходит из группы [chat].
     *
     * Только из группы и только не создателю: диалог на двоих покинуть нельзя — он остался бы
     * с одним участником, а создателю выход закрыт правилом, иначе группа осталась бы без
     * того, кто правит состав. У создателя есть другой способ её закончить — стереть целиком.
     *
     * После выхода наши слушатели диалога получают отказ по правам: доступ к нему пропал
     * вместе с членством. Снимать их — работа вызывающего, и снимать **до** записи, иначе
     * вышедший увидит собственный отказ как поломку.
     */
    suspend fun leave(chat: Chat): Result<Unit>
